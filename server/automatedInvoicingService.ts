...
export class AutomatedInvoicingService {
  ...
  private initializeDefaultRules() {
    ...
    console.log(`🔔 Initialized ${defaultReminders.length} default payment reminder rules`);
  }
  ...
  public startAutomatedInvoicing() {
    if (this.isRunning) {
      console.log("📋 Automated invoicing already running");
      return;
    }

    console.log("🚀 Starting automated invoicing service");
    this.isRunning = true;
  ...
  public async processAutomatedInvoicing() {
    try {
      console.log("🔄 Processing automated invoicing...");
  ...
  private async sendPaymentReminder(invoice: Invoice, rule: InvoiceReminderRule): Promise<boolean> {
    try {
      ...
      if (sent) {
        console.log(`📧 Sent ${rule.reminderType} reminder for invoice ${invoice.invoiceNumber} to ${client.email}`);
      }
  ...
  public addReminderRule(rule: Omit<InvoiceReminderRule, 'id' | 'createdAt'>): string {
    ...
    console.log(`🔔 Added payment reminder rule: ${fullRule.name}`);
    return id;
  }
