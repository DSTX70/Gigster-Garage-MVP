...
- **Agent KPI Tracking & Switchboard Automation** (Completed November 2025): Implemented comprehensive agent performance monitoring system with automated graduation tracking; added agentKpis database table tracking on-time milestone rate, gate escape rate, and incident counts; created Switchboard background service running every 60 minutes to auto-evaluate agents and promote those meeting graduation criteria (≥95% on-time, ≤1% gate escape, 0 incidents); added fourth "KPIs & Metrics" tab to Agent Management UI displaying live metrics with color-coded status badges (green/yellow/red) and manual Promote button; system gracefully handles missing database tables with helpful guidance messages; all sample KPI data initialized for the 6 internal agents (ITSA, SSK, Exec Orchestrator, Planner, Ledger, Sentinel)
